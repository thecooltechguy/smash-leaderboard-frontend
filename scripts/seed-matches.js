...
if (process.env.NODE_ENV === "production") {
  console.error(
    "❌ SAFETY CHECK: Seed script cannot run in production environment!"
  );
...
if (!databaseUrl.includes("localhost") && !databaseUrl.includes("test")) {
  console.error(
    "❌ SAFETY CHECK: Database URL does not appear to be local or test database!"
  );
  console.error(
  ...
  console.error(
    "This script should only run against local development databases."
  );
  process.exit(1);
}

console.log(
  "✅ Safety checks passed - running against local development database"
);
...
async function seedData() {
  ...
  try {
    ...
    console.log(
      `🎉 Successfully created ${matchesToCreate} matches with participants!`
    );
    console.log(
      '🔄 You should now be able to test the "Load More Matches" functionality'
    );
  } catch (error) {
